 ...
 * POST /api/analyze
 *
 * Proxies a video URL to the Railway backend, parses the response,
 * and returns a formatted Attentiq diagnostic report.
 *
 * Request body:
 *   { "url": "https://..." }
 *
 * Response (success):
 *   { "report": { text, data, partial } }
 *
 * Response (error):
 *   { "error": "...", "userMessage": "..." }
 */
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json(
      { error: "Invalid JSON body", userMessage: "Requête invalide." },
      { status: 400 }
    );
  }
    ...
    return NextResponse.json(
      {
        error: "MISSING_URL",
        userMessage:
          "Collez une URL TikTok publique ou passez par l'upload video.",
        needsUpload: true,
    ...
    return NextResponse.json(
      {
        error: "INTERNAL",
        userMessage:
          "Une erreur inattendue est survenue. Reessayez ou passez par l'upload video.",
        needsUpload: true,
        pipelineVersion: URL_PIPELINE_VERSION,
      },
      {
